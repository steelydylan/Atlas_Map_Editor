...
Atlas.DestLayer = Atlas.createClass(Atlas.Layer,{
	...
	duplicateLayer:function(id){
		...
		newmap.drawHisory = map.drawData.clone();
...
Atlas.downloadApp = Atlas.createClass(Atlas.App,{
	...
	downloadMap:function(map,path,chipSize){
		var fs = require("fs");
		var thismap = this.getChild({name:"map"});
		thismap.drawData = map.drawData.clone();
		thismap.img = map.img;
		thismap.setSpriteSize(chipSize,chipSize);
		thismap.setSize(chipSize,chipSize);
		var width = thismap.width*thismap.drawData[0].length;
		var height = thismap.height*thismap.drawData.length;
		this.setSize(width,height);
		this.setQuality(width,height);
		thismap.draw();
		fs.writeFileSync(path,this.toDataURL().replace(/^data:image\/png;base64,/, ""),"base64");
	},
});
...
Atlas.main(function(){
	...
	$("#destChooser").change(function(e){
		...
		var destLayer = dest.getChild({eventEnable:true,name:"dest"});
		var map = destLayer.getChild({name:"map"});
		var chipSize = parseInt($("#chipSize").val());
		$(this).val("");

		writer.downloadMap(map,path,chipSize);
	});
